import { Link } from "react-router";
import { menuItems, type MenuItem } from "../constants/menuItems";

const Header = () => {
  return (
    <header>
      <nav className="bg-teal-100 border-gray-200 px-4 lg:px-6 py-2.5">
        <div className="flex flex-wrap justify-between items-center mx-auto max-w-screen-xl">
          <Link to="/" className="flex items-center">
            <img
              src="../src/assets/da-vinci.png"
              className="mr-3 h-6 sm:h-9"
              alt="Flowbite Logo"
            />
            <span className="self-center text-xl font-semibold whitespace-nowrap">
              Da Vinci
            </span>
          </Link>
          <div
            className="hidden justify-between items-center w-full lg:flex lg:w-auto lg:order-1"
            id="mobile-menu-2"
          >
            <ul className="flex flex-col mt-4 font-medium lg:flex-row lg:space-x-8 lg:mt-0">
              {menuItems.map(({ id, path, label }: MenuItem) => (
                <li key={id}>
                  <Link to={path}>{label}</Link>
                </li>
              ))}
              <li>
                <Link
                  to="/users"
                  className="block py-2 pr-4 pl-3 text-gray-700 border-b border-gray-100 hover:opacity-70 lg:hover:bg-transparent lg:border-0 lg:hover:text-primary-700 lg:p-0"
                >
                  Users
                </Link>
              </li>
              <li>
                <Link
                  to="/posts"
                  className="block py-2 pr-4 pl-3 text-gray-700 border-b border-gray-100 hover:opacity-70 lg:hover:bg-transparent lg:border-0 lg:hover:text-primary-700 lg:p-0"
                >
                  Posts
                </Link>
              </li>
            </ul>
          </div>
        </div>
      </nav>
    </header>
  );
};

export default Header;
